...
namespace FpsManager
{
    ...
    public sealed class PeekMovement
    {
        ...
        public PeekMovement(int seed,DeploymentNavigation navigation,int movement=50,bool proactive=false,int composure=50,bool ambush=false)
        { nav=navigation; rng=new DeterministicRandom(seed); skill=Mathf.Clamp01(movement/100f);this.proactive=proactive;this.composure=Mathf.Clamp01(composure/100f);this.ambush=ambush; }
        ...
        public bool Step(float dt,Vector2 position,PlayerObjective order,Vector2 probe,bool contact,out Vector2 target,out Vector2 watch)
        {
            ...
            if(!Active)
            {
                ...
                Vector2 lateral=new Vector2(-forward.y,forward.x);
                bool found=false;
                if(proactive&&order.hasCover&&Vector2.Distance(position,order.cover)<4&&nav.Clear(position,order.cover)&&nav.SightClear(position,probe)&&!nav.SightClear(order.cover,probe))
                {home=order.cover;edge=position;found=true;}
            ...
            total+=dt;
            if(total>(proactive?12:8)) { phase=0; cooldown=2; return false; }
            watch=(proactive?nav.VisibleAimPoint(position,look):look)-position;
            if(phase==5||phase==6)
            {
                target=edge;
            ...
            if(proactive&&!nav.SightClear(exposed,look))exposed=edge;
