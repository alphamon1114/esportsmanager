...
public static class MatchFlowChecks
{
 ...
 public static void Run()
 {
  ...
  game.AdvanceFrame(2.9f);Check(game.Stage==MatchStage.Buying&&game.Combat.Shots==0,"Buying simulated combat");
  ...
  game.AdvanceFrame(2.9f);Check(game.Stage==MatchStage.Result&&game.CompletedRounds==1,"Result reset too early");
  ...
  game.AdvanceFrame(3.01f);Check(game.Stage==MatchStage.Live&&game.Director.Strategy==TeamStrategy.Defensive,"Next round strategy/start");
  End(game,game.TeamIndexOf(0));game.AdvanceFrame(3.01f);
  ...
  game.AdvanceFrame(3.01f);Check(game.Stage==MatchStage.Live,"Auto resume failed");
  int startingCt=game.CounterTerroristTeam;
  while(game.CompletedRounds<8){End(game,0);game.AdvanceFrame(3.01f);game.AdvanceFrame(3.01f);}
  Check(game.CounterTerroristTeam!=startingCt,"Halftime missing");
  End(game,0);game.AdvanceFrame(3.01f);
  ...
  Debug.Log("MATCH_FLOW_ALL_OK 3s result / 3s buy, automatic rounds, rewards, timeout queue/limit/expiry, strategy lock/application, halftime, map end, deuce rules, buying, new map");
 }
}
